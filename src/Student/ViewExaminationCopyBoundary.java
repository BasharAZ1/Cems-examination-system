...
/**
 * view student exams copy GUI
 * 
 * @author Bashar Ali & Samr Arkab
 *
 */
public class ViewExaminationCopyBoundary {

	@FXML
	private Button logoutViewExamCopyBtn;
	...
	@FXML
	private ImageView backViewExamCopyBtn;

	@FXML
	private Button logoutViewExamCopyBtn1;
	...
	private void initialize() {
		...
		// teacherLastname_subject_course_studentComment_
		// _questionText_ans1_ans2_ans3_ans4_chosenAns_correctAns_qScore_actualqScore

		ObservableList<String> list = FXCollections.observableArrayList();
		list.addAll("1", "2", "3", "4");

		gradeLbl.setText(MyClient.FinalGrade);
		gradeLbl.setStyle("-fx-font-family:Dubai Light; -fx-font-size: 14; -fx-text-fill: purple");
		...
		TeachCommLbl.setText(MyClient.StudentComments);
		TeachCommLbl.setStyle("-fx-font-family:Dubai Light; -fx-font-size: 14; -fx-text-fill: purple");
	...
	/**
	 * start stage for viewing copy of exam
	 * 
	 * @param primaryStage Stage object
	 * @throws Exception in case starting the stage was unsuccessful
	 */
	public void start(Stage primaryStage) throws Exception {
		Parent root = FXMLLoader.load(getClass().getResource("/Student/ViewExamination.fxml"));
		Scene scene = new Scene(root);
		primaryStage.getIcons().add(new Image(getClass().getResourceAsStream("/logo.png")));
		primaryStage.setTitle("View examination copy form");
	...
	/**
	 * back to student view grade GUI
	 * 
	...
	void back(ActionEvent event) {
		...
		StudentViewGradeBoundary svg = new StudentViewGradeBoundary();
		Stage ps = new Stage();
		try {
			svg.start(ps);
		} catch (Exception e) {
