...
/**
 * start final phase of adding exam GUI, preview for final save
 * 
 * @author Bashar Ali & Samr Arkab
 *
 */
public class PreviewAddExamBoundary {
	...
	private void initialize() {
		...
		teacherComments.setText("Comment for teachers: " + decodeMsg[9]);
		teacherComments.setStyle("-fx-font-family:Dubai Light; -fx-font-size: 10; -fx-text-fill: purple");

		studentComments.setText("Comment for students: " + decodeMsg[8]);
		studentComments.setStyle("-fx-font-family:Dubai Light; -fx-font-size: 10; -fx-text-fill: purple");

		int j = 0;
		for (int i = 0; j < AddExamsBoundary.questable.size(); i += 5) {
			Label questionLb = new Label(AddExamsBoundary.questable.get(j).getQuestionText());
			questionLb.setStyle("-fx-font-family:Dubai Light; -fx-font-size: 16; -fx-text-fill: purple");

			Label scoreLb = new Label(
					"(" + Integer.toString(AddExamsBoundary.questable.get(j).getScore()) + " points)");
			...
			String ans1 = new String(AddExamsBoundary.questable.get(j).getAnswer1().getAnswerText());
			String ans2 = new String(AddExamsBoundary.questable.get(j).getAnswer2().getAnswerText());
			String ans3 = new String(AddExamsBoundary.questable.get(j).getAnswer3().getAnswerText());
			String ans4 = new String(AddExamsBoundary.questable.get(j).getAnswer4().getAnswerText());
			...
			int correctAns = AddExamsBoundary.questable.get(j).getCorrectAns();
	...
	/**
	 * start preview for built exam GUI
	 * 
	 * @param primaryStage Stage object
	 * @throws Exception in case starting the stage was unsuccessful
	 */
	public void start(Stage primaryStage) throws Exception {
		Parent root = FXMLLoader.load(getClass().getResource("/Exam/PreviewAddExam.fxml"));
	...
	void edit(ActionEvent event) {
		...
		AddExam2Boundary.pS.show();
	}

	/**
	 * save exam in data base
	 * 
	...
	void saveAction(ActionEvent event) {
		 ...
		 */
		ClientUI.chat.accept(AddExam2Boundary.temp);
		AddExam2Boundary.flag2 = 0;
		AddExam2Boundary.temp = "";
